	...
	(d.prototype = {
		...
		_init: function() {
			...
			this.$element.parent().hasClass('bootstrap-timepicker')
				...
				this.template !== !1
					? (this.$widget = a(this.getTemplate()).on('click', a.proxy(this.widgetClick, this)))
					: (this.$widget = !1),
				this.showInputs &&
					this.$widget !== !1 &&
		...
		getTemplate: function() {
			...
			switch ((this.showInputs
				? ((b = '<input type="text" class="bootstrap-timepicker-hour" maxlength="2"/>'),
					...
					(e = '<input type="text" class="bootstrap-timepicker-meridian" maxlength="2"/>'))
			...
			(f =
				...
				c +
				'</td> ' +
				(this.showSeconds ? '<td class="separator">:</td><td>' + d + '</td>' : '') +
				(this.showMeridian ? '<td class="separator">&nbsp;</td><td>' + e + '</td>' : '') +
		...
		getTime: function() {
			return '' === this.hour
				...
				: this.hour +
					...
					(this.showSeconds
						? ':' + (1 === this.second.toString().length ? '0' + this.second : this.second)
						: '') +
				...
				this.position >= 0 && this.position <= 2
					...
					: this.position >= 3 && this.position <= 5
						...
						: this.position >= 6 && this.position <= 8
							...
							: this.position >= 9 && this.position <= 11 && this.highlightMeridian();
		},
		...
		place: function() {
			if (!this.isInline) {
				...
				this.$widget.removeClass(
					...
					'auto' !== this.orientation.x
						...
						: (this.$widget.addClass('timepicker-orient-left'),
							j.left < 0 ? (m -= j.left - e) : j.left + c > f && (m = f - c - e));
				var o,
					p,
				...
				'auto' === q &&
					...
					'top' === q ? (n += k) : (n -= d + parseInt(this.$widget.css('padding-top'), 10)),
		...
		setDefaultTime: function(a) {
			if (this.$element.val()) this.updateFromElementVal();
				...
				0 !== e &&
					...
					60 === e && ((d += 1), (e = 0))),
					0 !== d &&
						((d = Math.ceil(b.getMinutes() / this.minuteStep) * this.minuteStep),
						60 === d && ((c += 1), (d = 0))),
					this.showMeridian &&
						(0 === c ? (c = 12) : c >= 12 ? (c > 12 && (c -= 12), (f = 'PM')) : (f = 'AM')),
					(this.hour = c),
					(this.minute = d),
					(this.second = e),
					(this.meridian = f),
		...
		setTime: function(a, b) {
			...
			if ('object' == typeof a && a.getMonth)
				(e = a.getHours()),
					(f = a.getMinutes()),
					(g = a.getSeconds()),
					this.showMeridian && ((h = 'AM'), e > 12 && ((h = 'PM'), (e %= 12)), 12 === e && (h = 'PM'));
			else {
				...
				(f = d[1] ? d[1].toString() : ''),
					...
					this.showMeridian
						...
						: 12 > e && 2 === c
							? (e += 12)
							: e >= this.maxHours ? (e = this.maxHours - 1) : (0 > e || (12 === e && 1 === c)) && (e = 0);
			}
			(this.hour = e),
				this.snapToStep
		...
		showWidget: function() {
			this.isOpen ||
				...
				'modal' === this.template && this.$widget.modal
					? this.$widget.modal('show').on('hidden', a.proxy(this.hideWidget, this))
			...
			return (
				...
				this.each(function() {
					...
					f ||
						e.data(
							'timepicker',
							(f = new d(this, a.extend({}, a.fn.timepicker.defaults, g, a(this).data())))
						),
